...
def plot_mse_parameters(axes, K1, A, B, Q, mu0, Q0, C, d, R, true_A, true_B, true_Q, true_mu0, true_Q0, true_C, true_d, true_R):
    ...
    axes.scatter(range(11), [mse(A[:K1,:K1], true_A[:K1,:K1]), mse(A[K1:,:K1], true_A[K1:,:K1]), mse(A[K1:,K1:], true_A[K1:,K1:]), mse(B[:K1], true_B[:K1]), mse(Q, true_Q), mse(mu0, true_mu0), mse(Q0, true_Q0), mse(C[:,:K1], true_C[:,:K1]), mse(C[:,K1:], true_C[:,K1:]), mse(d, true_d), mse(R, true_R)])
    axes.set_xticks(range(11), ['A11', 'A21', 'A22', 'B1','Q', 'mu0', 'Q0', 'C1', 'C2', 'd', 'R'])
